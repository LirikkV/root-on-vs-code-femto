...
TString Output_File = "/home/kirill/root-on-vs-code/Femto_output/out_Au_Au200_15_04_FMR_proseed.root";
...
for (Int_t iCh = 0; iCh < N_Charge; iCh++)
{
    for (Int_t iCent = 0; iCent < N_Bins_Centr; iCent++)
    {
        for (Int_t iKt = 0; iKt < N_Bins_Kt; iKt++)
        {
            ...
            //3D:
            h_Arr_3D[0][iCh][iCent][iKt] = (TH3F* )f->Get(Form("%s_%i_%i_%i_%i",hist_3D_Name.Data(),0,iCh,iCent,iKt));
            h_Arr_3D[1][iCh][iCent][iKt] = (TH3F* )f->Get(Form("%s_%i_%i_%i_%i",hist_3D_Name.Data(),1,iCh,iCent,iKt));
            h_Arr_3D[2][iCh][iCent][iKt] = (TH3F* )f->Get(Form("%s_%i_%i_%i_%i",hist_3D_Name.Data(),2,iCh,iCent,iKt));
...
//3D with bins:
std::array<std::array<std::array<std::array<TH3F*,N_Bins_Kt>,N_Bins_Centr>,N_Charge>, h_CF_3D_non_norm = {}; 
std::array<std::array<std::array<std::array<TH1F*, 3 >,N_Bins_Kt>,N_Bins_Centr>,N_Charge> h_CF_3D_non_norm_Project_OSL = {}; 
                                        //  h_CF_3D_non_norm[Charge][Centr][Kt][OSL] 0 - out 1 - side 2 - long
...
for (Int_t iCh = 0; iCh < N_Charge; iCh++)
{
    for (Int_t iCent = 0; iCent < N_Bins_Centr; iCent++)
    {
        for (Int_t iKt = 0; iKt < N_Bins_Kt; iKt++)
        {
            h_CF_3D_non_norm[iCh][iCent][iKt] = (TH3F*)h_Arr_3D[0][iCh][iCent][iKt]->Clone(Form("CF_3D_non_norm_%i_%i_%i",iCh,iCent,iKt) );
            for(Int_t iOSL =0; iOSL<2;iOSL++)
            {
            
            h_CF_3D_non_norm[iCh][iCent][iKt]->Divide(h_Arr_1D[1][iCh][iCent][iKt]);
            h_CF_3D_non_norm[iCh][iCent][iKt]->SetTitle(Form("CF_1D_non_norm Charge: %i Centrality: %i K_t: %i",iCh,iCent,iKt));
            h_CF_3D_non_norm[iCh][iCent][iKt]->GetXaxis()->SetTitle("q_inv");
            h_CF_3D_non_norm[iCh][iCent][iKt]->GetYaxis()->SetTitle("CF");

            h_CF_3D_non_norm_Project_OSL[iCh][iCent][iKt][iOSL] = (TH1F*)h_CF_3D_non_norm[iCh][iCent][iKt]->Project3D(OSL_xyz[iOSL]);

            

            h_CF_3D_non_norm_Project_OSL[iCh][iCent][iKt][iOSL]->Sumw2();
            }
        }
    }
}
...
hA_Plus->Draw();
c1->SaveAs(Output_Folder+"PDF_data/" + "hA_Plus.pdf");
hB_Plus->Draw();
c1->SaveAs(Output_Folder+"PDF_data/" + "hB_Plus.pdf");
CF_Pi_Plus_non_nrom->Draw();
c1->SaveAs(Output_Folder+"PDF_data/" + "CF_Pi_Plus_non_norm.pdf");
CF_Pi_Plus->Draw();
c1->SaveAs(Output_Folder+"PDF_data/" + "hCF_Pi_Plus.pdf");
hA_Plus_normalized->Draw();

c1->SaveAs(Output_Folder+"PDF_data/" + "hA_Norm_Pi_Plus.pdf");
gStyle->SetOptStat(0);
...
for (Int_t iCent = 0; iCent < N_Bins_Centr; iCent++)
{
    for (Int_t iKt = 0; iKt < N_Bins_Kt; iKt++)
    {
        ...
        h_CF_1D_non_norm[0][iCent][iKt]->GetXaxis()->SetRangeUser(0.0, 0.3);
        h_CF_1D_non_norm[0][iCent][iKt]->SetTitle(Form("CF_1D_Non_norm Charge %i, Centrality %i, K_t [%.2f,%.2f]", 0, iCent,
                                                       KtBins[iKt], KtBins[iKt + 1]));
        h_CF_1D_non_norm[0][iCent][iKt]->Draw();
    }
}

c1->SaveAs(Output_Folder +"PDF_data/" + "hA_B_difference.pdf");
