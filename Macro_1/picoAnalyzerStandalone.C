...
void Get_q_inv_q_osl(const double& px1,const double& py1,const double& pz1,const double& e1,
                        ...
                        double q[4])
{
  double dpx = px1 - px2; // dpx = q_x
  double dpy = py1 - py2; // dpy=q_y
  double dpz = pz1 - pz2; // dpz=q_z
  double de = e1 - e2;    // de=q_0
  ...
  double M_T = sqrt(P_0 * P_0 - P_z * P_z);

  double q_out = (P_x * dpx + P_y * dpy) / (P_T);
  double q_side = (P_x * dpy - P_y * dpx) / (P_T);
  double q_long = (P_0 * dpz - P_z * de) / (M_T);
...
//function for tracks shuffle:
std::vector<Int_t> Shuffle_vector(Int_t N)
{
  ...
  if(!Pions_4_momenta_hits_Arr.empty())
    {
      ...
      {
          double px1 =  Pions_4_momenta_hits_Arr[i].p4.Px();
          double py1 = Pions_4_momenta_hits_Arr[i].p4.Py();
          double pz1 = Pions_4_momenta_hits_Arr[i].p4.Pz();
                           ...
                           TH3F* hist_B_osl)
{
  ...
  for(size_t i=0;i<nEventsInQueue;i++)
  {
    ...
    {
      double px1 = new_Pions_Arr[j].p4.Px();
      double py1 = new_Pions_Arr[j].p4.Py();
      double pz1 = new_Pions_Arr[j].p4.Pz();
      double e1 = new_Pions_Arr[j].p4.E();
...
gRandom->SetSeed(42);
  ...
  //!!!! form -0.4 to 0.4 for q_osl; 10Mev bin
  TH3F *hA_Pi_Plus_q_osl = new TH3F("hA_Pi_Plus_q_osl","Num of CF Pi+Pi+ out, side, long;out;side;long",
                                      80,-0.4,0.4,80,-0.4,0.4,80,-0.4,0.4);
  TH3F *hA_Pi_Minus_q_osl = new TH3F("hA_Pi_Minus_q_osl","Num of CF Pi-Pi- out, side, long;out;side;long",
                                      80,-0.4,0.4,80,-0.4,0.4,80,-0.4,0.4);
  TH3F *hB_Pi_Plus_q_osl = new TH3F("hB_Pi_Plus_q_osl","Denum of CF Pi+Pi+ out, side, long;out;side;long",
                                      80,-0.4,0.4,80,-0.4,0.4,80,-0.4,0.4);
  TH3F *hB_Pi_Minus_q_osl = new TH3F("hB_Pi_Minus_q_osl","Denum of CF Pi-Pi- out, side, long;out;side;long",
                                      80,-0.4,0.4,80,-0.4,0.4,80,-0.4,0.4);
  ...
  TH2F *hSL_B = new TH2F("SL_vs_qinv_B",
			    ...
			    400,-0.,0.2,200,-0.5,1.);
  //FMR:
  TH1F *hFMR_A = new TH1F("FMR_A","Fraction of merged rows A;FMR;N",200,-1.,1);
  ...
  for(Long64_t iEvent=0; iEvent<events2read; iEvent++) {
    ...
    // Track loop
    //let's shuffle traks in event:
    std::vector<Int_t> v_shuffle = Shuffle_vector(nTracks);
...
//here TPC topology and some useful functions for FMR:
//_________________
